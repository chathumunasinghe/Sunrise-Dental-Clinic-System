...
public class StaffDAO {
    ...
    /**
     * Lists ADMIN/GUEST staff accounts for the "Manage Staff" screen.
     * Doctor logins (role=DENTIST) are deliberately excluded — they're
     * created and managed from the "Manage Doctors" screen instead, so a
     * doctor never shows up twice across two different staff lists.
     */
    public List<Staff> getAllStaff() {
        List<Staff> list = new ArrayList<>();
        String sql = "SELECT * FROM staff WHERE role <> 'DENTIST' ORDER BY staff_id DESC";
        try (PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                list.add(mapRow(rs));
