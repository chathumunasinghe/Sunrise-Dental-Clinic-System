...
`TreatmentTypeTest`, `StaffTest`, `DentistTest`) — verify constructors,
getters/setters, and role/status-check logic (`isAdmin()`, `isActive()`)
hold correct values with no transformation errors.
