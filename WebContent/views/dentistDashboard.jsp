...
<body>
    ...
    <div class="page-wrapper">
        ...
        <div class="stat-grid" style="grid-template-columns:repeat(3,minmax(0,1fr)); margin-bottom:20px;">
            <div class="stat-card"><div class="stat-copy"><small><%= request.getAttribute("selectedDate") == null ? "Total Assigned" : "On This Date" %></small><strong><%= total %></strong></div><span class="stat-icon"><i class="bi bi-calendar2-week"></i></span></div>
        ...
        <div class="card">
            <div class="staff-toolbar" style="margin-bottom:14px;">
                <div><h3 style="margin:0;"><i class="bi bi-calendar2-week"></i>&nbsp; Assigned Appointments</h3>
                <p style="margin:2px 0 0;"><%= request.getAttribute("selectedDate") == null ? "Showing your full schedule." : "Showing your schedule for " + request.getAttribute("selectedDate") + "." %></p></div>
            </div>
            <% if (request.getAttribute("error") != null) { %><div class="alert alert-error"><%= request.getAttribute("error") %></div><% } %>
            <form action="${pageContext.request.contextPath}/dentistDashboard" method="get" style="display:flex; gap:12px; align-items:flex-end; margin-bottom:18px; flex-wrap:wrap;">
                <div class="field" style="flex:1; max-width:260px; margin-bottom:0;">
                    <label>Search Schedule by Date</label>
                    <input type="date" class="form-control" name="date" value="<%= request.getAttribute("selectedDate") == null ? "" : request.getAttribute("selectedDate") %>" />
                </div>
                <button type="submit" class="btn" style="width:auto; padding:11px 24px;"><i class="bi bi-search"></i> Search</button>
                <% if (request.getAttribute("selectedDate") != null) { %>
                    <a class="btn btn-secondary inline-btn" style="text-decoration:none;" href="${pageContext.request.contextPath}/dentistDashboard"><i class="bi bi-x-circle"></i> Clear</a>
                <% } %>
            </form>

            <% if (myAppointments == null || myAppointments.isEmpty()) { %>
                <div class="empty-state"><i class="bi bi-calendar2"></i><%= request.getAttribute("selectedDate") == null ? "No appointments assigned to you yet." : "No appointments on this date." %></div>
            <% } else { %>
